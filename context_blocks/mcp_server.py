...
def report_gap(description: str, related_entities: list[str] | None = None, block: str = "") -> dict:
    """Record a KNOWLEDGE GAP. TRIGGER: you are UNSURE or confused, the block is contradictory, or you need information it doesn't have — the "I can't confidently answer this" signal. It's a gap you reasoned out (a contradiction, an unconfirmed assumption), not just a failed search: e.g. 'the block documents rate limits for the sync API but assumes none for the bulk API, and nothing reconciles which applies to a bulk resync.' This is the single most valuable thing you can produce — reasoned gaps don't show up as failed lookups, so without this they'd be lost. It's recorded as a first-class gap on the current work-effort and surfaces on the dashboard as the curation backlog. related_entities = the entity ids the gap touches. Requires an open work-effort (call begin_work_effort first)."""
    if not _current_we:
...
def log_finding(observation: str, reasoning: str = "", source: str = "", block: str = "") -> dict:
    """Record a FINDING in your investigation log — something you observed and what you make of it. Call this after each meaningful step so the work-effort becomes a full, ordered investigation log: what you did, what you saw, and your reasoning, in sequence. Use it for BOTH block conclusions ("matched the 'orders-not-ingested' pattern — its trigger keywords align") AND external observations that don't auto-log ("queried the order-service event log: 6/8 orders show status=PROCESSED but missing downstream"). `observation` = what you saw; `reasoning` = what it means / your read; `source` = where it came from (e.g. 'block:orders-not-ingested', 'ticketing:TICKET-42', 'metrics:logs'). Log when you CONCLUDE or OBSERVE — not for every trivial call (CB calls auto-log; this adds the reasoning and the external evidence). Requires an open work-effort."""
    if not _current_we:
